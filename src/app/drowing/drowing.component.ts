...
import { RulerSettingsModel, GridlinesModel, SnapSettingsModel, NodeModel, ConnectorModel, DiagramTools, ITextEditEventArgs } from '@syncfusion/ej2-angular-diagrams';

import { DiagramComponent } from '@syncfusion/ej2-angular-diagrams';
import { SafeHtml, SafeStyle, DomSanitizer } from '@angular/platform-browser';
import { DiagramService } from "src/app/service/diagram.service";
@Component({
  selector: 'app-drowing',
  templateUrl: './drowing.component.html',
  styleUrls: ['./drowing.component.css']
})
export class DrowingComponent  {
    sa:boolean
...
public rulerSettings: RulerSettingsModel = { showRulers: true, dynamicGrid: true }
    ...
    textEdit(args:ITextEditEventArgs){
        console.log("textEdit=>"+JSON.stringify(event));
        console.log(args.newValue); 
        console.log(args.oldValue); 
    }
  click(){
      console.log("click1777=>");
  }
    ...
    public documentClick(args: MouseEvent): void {
        ...
        if (target.className === 'image-pattern-style') {
          console.log("target.id=>" + JSON.stringify(target.id));
            switch (target.id) {
                case 'shape1':
        ...
        console.log("getNativeContent=> nodes: "+ JSON.stringify(this.nodes));
        console.log("drawing object:=>" +JSON.stringify(drawingObject));
        console.log("getNativeContent=> this.diagram: "+ this.diagram);
       
    };

    public diagramCreate(args: Object): void {
        this.diagram.drawingObject = { shape: { type: 'Basic', shape: 'Rectangle' } };
        console.log("diagramCreate=> nodes: "+ JSON.stringify(this.nodes));
        this.diagram.tool = DiagramTools.ContinuousDraw;
        
    }

nodeClicked(input){
    console.log("node clicked=> "+ JSON.stringify(input));

}
doubleClick(args: Object) { 
    console.log("SARI"); 
      if (args['source'].id!=undefined)
      {
           console.log(JSON.stringify(args['source'].offsetX)); 
