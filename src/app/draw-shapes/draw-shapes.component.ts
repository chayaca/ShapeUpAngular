...
export class DrawShapesComponent implements OnInit {
  ...
  updateShape(){
    this.orderPoints();
    this.currentShape.point=Array<Points>();
    this.currentShape.tempId=null;
    this.points.forEach(p => {
      let point=new Points();
      point.x=parseInt(p.x.toString());
      point.y=parseInt(p.y.toString());
      this.currentShape.point.push(point);
    });
    let s=new Shapes2();
    s.id=this.currentShape.id;
    s.area=false;
    s.unit=this.currentShape.unit;
    s.point=this.currentShape.point;
    this.shapeService.EditShape(s).subscribe((data:fullShape)=>{
      this.currentShape=data.shape;
      this.currentShape.point=data.points;
    ...
    this.scanvas.forEach(canvas => {
      ...
      this.sctx.beginPath();    
    this.sctx.moveTo(this.shapes[i].point[0].x*4, this.sctx.canvas.height-(this.shapes[i].point[0].y*4));
    console.log(this.shapes[i].point[0].x*4);
    console.log(this.sctx.canvas.height-(this.shapes[i].point[0].y*4));
    this.shapes[i].point.forEach(p=>{
      this.sctx.lineTo((p.x*4), (this.sctx.canvas.height-(p.y*4)));
      console.log((p.x*4));
      console.log((this.sctx.canvas.height-(p.y*4)));
    })
    this.sctx.lineTo(this.shapes[i].point[0].x*4, this.sctx.canvas.height-(this.shapes[i].point[0].y*4));
    console.log(this.shapes[i].point[0].x*4);
    console.log(this.sctx.canvas.height-(this.shapes[i].point[0].y*4));
    this.sctx.closePath();
  ...
  drawShape(id:number){
    ...
    this.shapes[id].point.forEach(p=>{
      this.drawLine(p.x, p.y, false);
    })
    this.drawLine(this.shapes[id].point[0].x, this.shapes[id].point[0].y, false);
  ...
  saveShape(){
    ...
    s.result=null;
    s.unit=this.unit;
    this.shapeService.AddShape(s).subscribe((data: fullShape)=>{
      this.currentShape=data.shape;
      this.currentShape.point=data.points;
      // this.shapeService.GetPoints(this.currentShape.id).subscribe((data:Points[])=>{
      // this.currentShape.points=data;
  ...
  ngOnInit() {
    ...
    this.drawCoordinates();
    let sh=Array<fullShape>();
    this.shapeService.GetFullShapes().subscribe((data:fullShape[])=>{
        sh=data.filter(s=>s.shape.area==false);
        sh.forEach(s=>{
          let shp=new Shapes();
          shp=s.shape;
          shp.point=s.points;
          shp.tempId=this.indexOfShape;
          this.indexOfShape++;
          this.shapes.push(shp);
          // this.shapeService.GetPoints(s.id).subscribe((data:Array<Points>)=>{
          //   console.log(data);
          //   s.points=data;
